...
public class ID3v1Tag implements ID3Tag {
     ...
     */
    public ID3v1Tag(File mp3) throws IOException {
        this.mp3 = mp3;
     ...
     */
    private boolean checkHeader(RandomAccessFile raf) throws IOException {
     ...
     *            the open file to read from
     */
    private void readTag(RandomAccessFile raf) throws IOException {

        raf.seek(raf.length() - TAG_SIZE);
        byte[] buf = new byte[TAG_SIZE];
        raf.readFully(buf);
        String tag = new String(buf, "ISO-8859-1");
        assert (tag.length() == TAG_SIZE);
    ...
    public byte[] getBytes() {
        ...
        try {
            System.arraycopy(TAG_START.getBytes("ISO-8859-1"), 0, tag,
                    bytesCopied, TAG_START.length());
            bytesCopied += TAG_START.length();
            System.arraycopy(title.getBytes("ISO-8859-1"), 0, tag, bytesCopied,
                    title.length());
            bytesCopied += TITLE_SIZE;
            System.arraycopy(artist.getBytes("ISO-8859-1"), 0, tag,
                    bytesCopied, artist.length());
            bytesCopied += ARTIST_SIZE;
            System.arraycopy(album.getBytes("ISO-8859-1"), 0, tag, bytesCopied,
                    album.length());
            bytesCopied += ALBUM_SIZE;
            System.arraycopy(year.getBytes("ISO-8859-1"), 0, tag, bytesCopied,
                    year.length());
            bytesCopied += YEAR_SIZE;
            System.arraycopy(comment.getBytes("ISO-8859-1"), 0, tag,
                    bytesCopied, comment.length());
        } catch (UnsupportedEncodingException ex) {
            // ISO-8859-1 is one of the encodings
            // that JVMs must support
            throw new Error(ex);
        }
     ...
     */
    @Override
    public void removeTag(boolean b) throws IOException {
        removeTag();
    }
    ...
    /**
     * Return the track field of the tag. This method will return
     * 0 if the track byte is either 0x00 or 0xFF.
     * 
     * @return an integer between 0 and 254 (inclusive)
     */
    public int getTrack() {
        return (track == 255) ? 0 : track;
    }
