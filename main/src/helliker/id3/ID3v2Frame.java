...
public class ID3v2Frame {
    ...
    /**
     * Holds the possible ID3 tag encodings.
     * 
     */
    static final String[] ENC_TYPES = { "ISO-8859-1", "UTF16", "UTF-16BE",
            "UTF-8" };
    /**
     ...
     * when the audio is completely replaced with other audio data.</br>
     * <tableborder="0">
     * 
     * <tr>
     * 
     ...
     * Every frame with the same group identifier belongs to the same group.<br/>
     * 
     * <tableborder="0">
     * 
     * <tr>
     ...
     * 
     * <td>
     * Frame does not contain group information</td>
     * 
     * </tr>
     * 
     * <tr>
     ...
     * 
     * <td>
     * Frame contains group information</td>
     * 
     * </tr>
     * 
     * </table>
     ...
     * Length Indicator' byte MUST be included in the frame.<br/>
     * 
     * <table>
     * 
     * <tr>
     ...
     * href="ftp://ftp.isi.edu/in-notes/rfc1950.txt" title="P. Deutsch, Aladdin
     * Enterprises & J-L. Gailly, 'ZLIB Compressed Data Format Specification
     * version 3.3', RFC 1950, May 1996.">[ZLIB]</a> deflate method. If set,
     * this requires the 'Data Length Indicator' bit to be set as well.</td>
     * 
     * </tr>
     * 
     * </table>
     ...
     * frame. See section 6 for details on unsynchronisation. If this flag is
     * set all data from the end of this header to the end of this frame has
     * been unsynchronised. Although desirable, the presence of a 'Data Length
     ...
     * 
     * <td>
     * Frame has not been unsynchronised.</td>
     * 
     * </tr>
     * 
     * <tr>
     ...
     * 
     * <td>
     * Frame has been unsyrchronised.</td>
     * 
     * </tr>
     * 
     * </table>
     ...
     * 
     * <td>
     * There is no Data Length Indicator.</td>
     * 
     * <tr>
     * 
     * <td>
     ...
     * 
     * <td>
     * A data length Indicator has been added to the frame.</td>
     * 
     * </tr>
     * 
     * </table>
    ...
    public ID3v2Frame(String id, byte[] data, boolean tagAlterDiscard,
            boolean fileAlterDiscard, boolean readOnly, boolean grouped,
            boolean compressed, boolean encrypted, boolean unsynchronised,
    ...
    /**
     * Returns a byte array representation of this frame that can be written to
     * a file. Includes the header and data.
     * 
     ...
     * into a string. If this frame does not contain textual information, an
        ...
        if (frameData.length > 1) {
            try {
                if ((id.charAt(0) == 'T')
                    ...
                    int offset = getValOffset(frameData);
                    if (offset < frameData.length) {
                        str = new String(frameData, offset, frameData.length
                                - offset, ENC_TYPES[0]);
                    }
                } else if (id.charAt(0) == 'W') {
                    str = new String(frameData, 0, frameData.length,
                            ENC_TYPES[0]);
                } else if (id
                        .equals(ID3v2Frames.UNSYNCHRONISED_LYRIC_TRANSCRIPTION)
                        || id.equals(ID3v2Frames.COMMENTS)
                        || id.equals(ID3v2Frames.TERMS_OF_USE)) {
                    int offset = getValOffset(frameData);
                    if (offset < frameData.length) {
                        str = getDecodedString(frameData, 0, offset);
                    }
                }
    ...
    /**
     * Returns the index of the first non-null byte in framedata that occurs
     * after the null-terminated string starting at offset. Whether the
     * null-terminated string must be terminated by one or two null bytes
     * depends on the encoding byte found at framedata[eindex].
     */
    static int getValOffset(byte[] bdata, int eindex, int offset) {
        int ienc = bdata[eindex];
        // skip anything that precedes (double) null terminator
        boolean singleNullByte = (ienc != 1 && ienc != 2);
        int offs = offset;
        boolean lastByteWasNull = false;
        for (; offs < bdata.length; offs++) {
            boolean byteIsNull = (bdata[offs] == 0x00);
            if (byteIsNull && (singleNullByte || lastByteWasNull)) {
                offs++;
                break;
            }
            lastByteWasNull = byteIsNull;
        }
        return offs;
    }

    private static int getValOffset(byte[] bdata) {
        return getValOffset(bdata, 0, 1);
    }
     ...
     * the frame and returns the 2 byte array of flags to be added to the
     * header.
     * 
     ...
     * byte in the array should contain the type of encoding. Where it is
     * located is specifed by the eIndex parameter. If an invalid type of
     * encoding is found, the empty string is returned.
     * 
     * @param b
    ...
    /**
     * Returns true if this frame should have the file alter preservation bit
     * set by default.
     * 
